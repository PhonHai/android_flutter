...
## 协程的本质：是任务调度机制，不是轻量级线程

官方文档说 "Coroutine is a lightweight thread"，很多人就直译为「协程就是轻量级线程」——**这是一个严重的误解**。

更准确的定义：

> **协程是一种任务调度机制，不是 CPU 执行资源。**

- **线程**：真正执行代码的东西（CPU → 线程 → 代码执行），是操作系统层面的执行单元
- **协程**：必须依附在线程上运行的任务单元，是用户态的调度单位

完整链路：

```
Coroutine → Continuation → Dispatcher → Thread Pool → Thread
```

| 角色 | 职责 |
|------|------|
| 线程（Thread） | 提供执行能力，OS 调度 |
| 协程（Coroutine） | 管理任务状态：何时执行、暂停、恢复 |

**核心结论**：协程不是让程序拥有更多线程，而是**让线程能够更高效地运行更多任务**。一个线程跑几万个协程是常态——协程切换只是保存/恢复状态（几 KB），远没有线程切换（保存寄存器、切换内核栈、TLB 刷新）昂贵。
...
## delay 为什么不阻塞线程：挂起 vs 阻塞

这是理解协程优势最直观的例子：

| | `Thread.sleep` | `delay` |
|------|-------------|-------|
| 本质 | 线程直接睡眠，一直占用 | 协程保存状态，释放线程 |
| 后果 | 睡着的线程不能干任何活 | 协程挂起时，线程可执行其他协程 |
| 资源 | 浪费线程资源 | 高效利用线程 |

```kotlin
// 两个协程可能跑在同一个线程上
launch { delay(1000); println("A") }  // 挂起时线程被释放
launch { delay(500);  println("B") }  // B 先输出，因为只等 500ms
// 输出顺序：B → A（两个协程都只挂起不阻塞，互不耽误）
```

**关键洞察**：`delay` 本质是"向调度器登记一个定时回调"，协程挂起后线程立刻被回收继续跑别的协程。1 秒后调度器唤醒这个协程，把它放回线程继续执行。整个过程**线程没有被阻塞任何时间**。
...
|------|--------|-----------|
| 本质 | 执行资源 | 任务调度机制 |
| 创建成本 | 高（~1 MB 栈空间 + 内核对象） | 低（~几 KB 的状态机对象） |
| 调度者 | OS 内核 | Coroutine Scheduler（用户态） |
| 切换成本 | 高（寄存器保存、内核栈切换、TLB 刷新） | 低（只保存/恢复 label + 局部变量） |
| 数量上限 | 有限（几百到几千） | 大量（数万） |
| 暂停方式 | 阻塞（线程睡眠，不可用） | 挂起（释放线程，非阻塞） |
| 并发模型 | 抢占式（OS 时间片） | 协作式（主动挂起点） |

> 💡 **一句话**：协程让少量的线程跑大量的任务——挂起时线程不丢失，可以立刻去执行其他协程。

## 面试速查：高频反直觉问题

> **Q: `launch` 会创建线程吗？**
>
> A: **不会。** `launch` 只是创建了一个 Coroutine 对象并注册到调度器。线程是 Dispatcher 的线程池里已有的。

> **Q: `suspend` 会切线程吗？**
>
> A: **不会。** `suspend` 只标记"这个函数可以被挂起"，切线程是由 `withContext(Dispatchers.XXX)` 或 `launch(Dispatchers.XXX)` 决定的。

> **Q: 什么时候真正发生线程切换？**
>
> A: 显式指定不同 Dispatcher 时——`withContext(Dispatchers.IO)` 或 `launch(Dispatchers.Default)`。

> **Q: `CoroutineExceptionHandler` 为什么捕获不到 `async` 的异常？**
>
> A: `async` 的异常被包装在 `Deferred` 中，只有调用 `await()` 时才会抛出。而 `await()` 抛出异常时，协程可能已经离开了 Handler 的作用范围。所以 Handler 只能捕获 `launch` 中直接抛出的未捕获异常。
...
> **Q: 协程是轻量级线程吗？为什么"轻量"？**
>
> A: 不完全是。更准确的说法是：**协程是一种任务调度机制，不是 CPU 执行资源**。它轻量在：
> - **用户态调度**：由 Kotlin 编译器生成的状态机 + CoroutineScheduler 调度，不经过 OS 内核
> - **挂起不阻塞线程**：协程挂起时释放线程去执行其他协程，线程利用率极高
> - **创建成本低**：启动一个协程 ≈ 创建一个状态机对象（几 KB），启动一个线程 ≈ 创建 OS 线程（~1 MB 栈 + 内核对象）

> **Q: 协程挂起时，线程在干什么？**
>
> A: **线程被释放了，去跑其他协程了。** 这是协程和线程阻塞的本质区别：
> - `Thread.sleep(1000)`：线程睡眠 1 秒，什么也不干，CPU 资源浪费
> - `delay(1000)`：协程挂起，线程立刻被回收继续执行其他协程，1 秒后调度器唤醒这个协程
> - 这就是为什么一个线程可以跑成千上万个协程——挂起 = "你去忙别的，我好了再回来"
...
> **Q: `launch` 会创建线程吗？`suspend` 会切线程吗？**
>
> A: 两个答案都是**不会**。`launch` 只是创建 Coroutine 对象；`suspend` 只是标记"允许挂起"。真正决定在哪个线程运行的是 `Dispatchers`，只有 `withContext(Dispatchers.IO)` 或 `launch(Dispatchers.Default)` 这类显式指定调度器时才会发生线程切换。

> **Q: CoroutineExceptionHandler 为什么捕获不到 async 的异常？**
>
> A: `async` 的异常被包装在 `Deferred` 中，只有调用 `await()` 时才抛出。而 Handler 只能在它所在的 scope 范围内捕获异常。最佳实践：用 `supervisorScope` + `try/catch` 包裹 `await()`。
...
## 冷流的本质：什么是「冷流」，为什么 Flow 是冷流

### 一、什么是冷流

一句话：**「按需生产，没有消费者就不工作」**。

- **冷流**：数据不会被主动生产，直到有消费者订阅它；每订阅一次，就从头执行一次。
- **热流**：数据一直生产，不管有没有人订阅——像广播电台，不管你有没有打开收音机，电台一直在广播。

### 二、为什么说 Flow 是冷流（两个核心原因）

#### 1. 惰性执行

`flow { ... }` 里的代码，在调用 `collect` 之前，**一行都不执行**：
...
fun main() = runBlocking {
    ...
    println("Flow 定义好了，但还没收集")
    // 这里 sleep 3 秒，Flow 里的代码也不会运行
    myFlow.collect { println("收到: $it") }
}
```
...
```text
Flow 定义好了，但还没收集
Flow 开始执行了！  ← 只有 collect 调用时才出现
收到: 1
收到: 2
```

如果 Flow 是热流（如 `StateFlow`），数据在定义那一刻就可能开始准备；普通 Flow 必须等 `collect` 触发。

#### 2. 独立性

每次 `collect` 都是全新的执行。两个消费者订阅同一个 Flow，各自拿到独立的数据流，互不干扰：

```kotlin
val myFlow = flow {
    println("=== 执行一次新的 Flow ===")
    emit(Random.nextInt(0, 100))   // 发射随机数
}
fun main() = runBlocking {
    myFlow.collect { println("消费者1: $it") }
    myFlow.collect { println("消费者2: $it") }
}
```
...
=== 执行一次新的 Flow ===
消费者1: 42
=== 执行一次新的 Flow ===  ← 第二次 collect 又重新执行了一遍代码块
消费者2: 88
```

**结论**：Flow 像**函数调用**，每次调用（collect）都重跑一遍逻辑；热流像**变量**，大家看到的是同一个值。

> Java 对照：非常像 Java 8 的 `Stream`——不调用终止操作（如 `collect()`/`reduce()`），中间操作一行都不执行。区别是 Flow 是**异步的、可挂起的** Stream，能跨线程发射。
...
- **Flow（冷流）= 点播视频**
  - 不点播放键，视频不会自己播放
  - 什么时候点播放，什么时候开始
  - 朋友在他家点同一个视频，是从头开始看的，和你不在一个进度（独立执行）
- **StateFlow / SharedFlow（热流）= 电视直播**
  - 电视台一直在播放（一直在发射数据）
  - 你打开电视，看到的是当前画面（实时数据）
  - 朋友打开电视，看的也是当前画面，和你同步

### 四、为什么 Kotlin 设计成冷流

1. **资源节约**：很多时候定义数据流只是为了 UI 显示，用户还没打开页面或页面已销毁，数据流就不应该跑。冷流天然支持「用则生，不用则灭」，避免浪费 CPU 和内存。
2. **上下文安全**：因为 Flow 是冷的，所以它可以在构建时通过 `flowOn` 随意切换线程，并且完全遵守协程的结构化并发——它像一个可以随时暂停、恢复、取消的任务。

### 五、Flow 能变成热流吗

可以。Kotlin 提供了 `StateFlow` 和 `SharedFlow` 把冷流「加热」：

- **StateFlow**：把它想象成一个「容器」，不管有没有人看，容器里始终装着一个最新的值；只要有人来取，就给他这个值——它就是热的。
- **shareIn / stateIn 操作符**：可以把一个冷的 Flow 转换成热的 StateFlow/SharedFlow。

> 详见下一章「六、StateFlow / SharedFlow — 热流」。
